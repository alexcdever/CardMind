...
//!
//! let pool_store = Arc::new(Mutex::new(PoolStore::new_in_memory()?));
...
impl P2PSyncService {
    ...
    fn verify_pool_hash_with_store(
        ...
        match Self::resolve_pool_hash_with_store(pool_store, pool_id) {
            Ok(local_hash) => local_hash == remote_hash,
            Err(err) => {
                warn!("pool_hash 校验失败: {}", err);
                false
...
mod tests {
    ...
    async fn it_should_reject_peer_on_pool_hash_mismatch() {
        ...
        let pool_a = Pool::new(&pool_id, "测试池A", "secretkey-a");
        ...
        let pool_b = Pool::new(&pool_id, "测试池B", "secretkey-b");
