...
pub async fn start_loopback_server(
    ...
    let addr: SocketAddr = "127.0.0.1:0".parse().map_err(|e| {
        crate::models::error::CardMindError::Io(format!("Invalid address: {}", e))
    })?;
...
mod tests {
    ...
    async fn http_server_starts_with_loopback_address() {
        ...
        let service = Arc::new(
            BackendService::new(dir.path().to_str().unwrap()).unwrap()
        );
