...
**Test File** | **测试文件**: `test/widgets/settings_panel_test.dart`

**Widget Tests** | **Widget 测试**:
- `it_should_show_current_theme_mode()` - Display theme mode | 显示主题模式
- `it_should_toggle_theme()` - Toggle theme | 切换主题
- `it_should_show_auto_sync_preference()` - Auto-sync preference | 自动同步偏好
- `it_should_show_sync_frequency_options()` - Sync frequency | 同步频率
- `it_should_show_app_version()` - App version | 应用版本
- `it_should_show_app_name_description()` - App name & description | 应用名称和描述
- `it_should_navigate_to_device_manager()` - Navigate to devices | 导航到设备
- `it_should_clear_local_cache()` - Clear cache | 清除缓存
- `it_should_export_data()` - Export data | 导出数据
- `it_should_show_privacy_policy()` - Privacy policy | 隐私政策
- `it_should_show_terms_of_service()` - Terms of service | 服务条款
- `it_should_group_settings_by_sections()` - Section grouping | 部分分组

**Acceptance Criteria** | **验收标准**:
- [ ] All widget tests pass | 所有 Widget 测试通过
- [ ] Theme switching works correctly | 主题切换正常工作
- [ ] Sync settings are functional | 同步设置功能正常
- [ ] Data management actions work reliably | 数据管理操作可靠工作
