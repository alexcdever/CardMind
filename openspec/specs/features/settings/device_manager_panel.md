...
**Related Tests** | **相关测试**: `test/widgets/device_manager_panel_test.dart`
...
- **WHEN** rendering the device manager panel
- **操作**：渲染设备管理面板
...
- **WHEN** rendering a device in the list
- **操作**：在列表中渲染设备
...
- **WHEN** a paired device is offline
- **操作**：配对设备处于离线状态
...
- **WHEN** a paired device is online
- **操作**：配对设备处于在线状态
...
**Test File** | **测试文件**: `test/widgets/device_manager_panel_test.dart`

**Widget Tests** | **Widget 测试**:
- `it_should_show_current_device_card()` - Display current device | 显示当前设备
- `it_should_show_device_online_status()` - Online status | 在线状态
- `it_should_enable_device_name_editing()` - Enable editing | 启用编辑
- `it_should_save_device_name()` - Save name | 保存名称
- `it_should_show_paired_devices()` - Display paired devices | 显示配对设备
- `it_should_show_empty_state()` - Empty state | 空状态
- `it_should_add_new_device()` - Add device | 添加设备
- `it_should_remove_paired_device()` - Remove device | 移除设备
- `it_should_prevent_removing_current_device()` - Prevent self-removal | 防止移除自身
- `it_should_show_device_type_icons()` - Device type icons | 设备类型图标
- `it_should_show_last_seen_for_offline()` - Last seen timestamp | 上次可见时间戳
- `it_should_show_online_now()` - Online now indicator | 当前在线指示器

**Acceptance Criteria** | **验收标准**:
- [ ] All widget tests pass | 所有 Widget 测试通过
- [ ] Device name editing works correctly | 设备名称编辑正常工作
- [ ] Device pairing/unpairing flows are smooth | 设备配对/取消配对流程流畅
- [ ] Status indicators are accurate | 状态指示器准确
