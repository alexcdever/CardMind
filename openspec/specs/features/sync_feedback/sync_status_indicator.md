...
- `it_should_show_synced_status()` - Display synced state | 显示已同步状态
- `it_should_show_syncing_status_with_animation()` - Display syncing state | 显示同步中状态
- `it_should_show_pending_status()` - Display pending state | 显示待同步状态
- `it_should_show_error_status()` - Display error state | 显示错误状态
- `it_should_show_connected_devices_count()` - Show device count | 显示设备数量
- `it_should_show_last_sync_time()` - Show last sync time | 显示上次同步时间
- `it_should_open_details_on_tap()` - Open details on tap | 点击打开详情
- `it_should_update_within_1_second()` - Update within 1s | 1秒内更新
- `it_should_animate_state_transitions()` - Animate transitions | 状态过渡动画
