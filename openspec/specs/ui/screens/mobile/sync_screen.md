...
**Related Tests**: `test/screens/sync_screen_test.dart`
...
This specification defines a dedicated mobile screen showing comprehensive synchronization information, device management, and sync controls.
...
- **GIVEN**: user navigates to sync screen
- **前置条件**：用户导航到同步屏幕
...
- **前置条件**：自动同步已启用
- **WHEN**: user configures sync frequency
- **操作**：用户配置同步频率
...
The system SHALL support pull-to-refresh gesture to update device list and sync status.
...
The system SHALL provide a floating action button for quick manual sync.
...
**Test File**: `test/screens/sync_screen_test.dart`
