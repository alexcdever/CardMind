...
**Related Tests**: `test/screens/card_detail_screen_test.dart`
...
This specification defines the mobile card detail screen that shows the complete card content with all metadata and editing capabilities.

本规格定义了移动端卡片详情屏幕，显示完整的卡片内容及所有元数据和编辑功能。
...
- **GIVEN**: user navigates to card detail screen
- **前置条件**：用户导航到卡片详情屏幕
...
- **GIVEN**: card detail screen is displayed
- **前置条件**：卡片详情屏幕已显示
...
- **GIVEN**: user is viewing card details
- **前置条件**：用户正在查看卡片详情
...
- **GIVEN**: user has made edits to the card
- **前置条件**：用户已对卡片进行编辑
...
The card detail screen SHALL use full-screen layout on mobile devices.
...
The system SHALL support swipe gestures for navigation between cards.
...
**Test File**: `test/screens/card_detail_screen_test.dart`
**测试文件**: `test/screens/card_detail_screen_test.dart`

**Screen Tests**:
**屏幕测试**:
- `it_should_display_full_title()` - Display title
- `it_should_display_full_title()` - 显示标题
- `it_should_show_complete_content()` - Show content
- `it_should_show_complete_content()` - 显示内容
- `it_should_show_metadata()` - Show metadata
- `it_should_show_metadata()` - 显示元数据
- `it_should_enter_edit_mode()` - Enter edit mode
- `it_should_enter_edit_mode()` - 进入编辑模式
- `it_should_save_changes()` - Save changes
- `it_should_save_changes()` - 保存更改
- `it_should_display_tags()` - Display tags
- `it_should_display_tags()` - 显示标签
- `it_should_add_tag()` - Add tag
- `it_should_add_tag()` - 添加标签
- `it_should_remove_tag()` - Remove tag
...
- `it_should_share_card()` - Share card
- `it_should_share_card()` - 分享卡片
- `it_should_show_sync_status()` - Show sync status
