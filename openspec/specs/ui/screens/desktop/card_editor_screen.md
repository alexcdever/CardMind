...
**Related Tests**: `test/screens/card_editor_screen_desktop_test.dart`
...
This specification defines the desktop card editor screen, providing an inline or side-panel card editing experience optimized for desktop workflows with multi-tasking support.
...
The system SHALL offer an inline or side-panel card editing experience optimized for desktop workflows.
...
- **GIVEN**: user selects a card from the list
- **前置条件**：用户从列表中选择卡片
- **WHEN**: editor opens with an existing card
- **操作**：编辑器打开现有卡片
- **THEN**: the system SHALL pre-populate title and content fields in the editor panel
- **预期结果**：系统应在编辑器面板中预填充标题和内容字段
- **AND**: load existing tags
- **并且**：加载现有标签
- **AND**: keep the card list visible
- **并且**：保持卡片列表可见
...
- **前置条件**：用户发起新卡片创建
- **WHEN**: editor opens for new card creation
- **操作**：编辑器打开以创建新卡片
...
- **并且**：显示成功确认
- **AND**: keep editor open for continued editing
- **并且**：保持编辑器打开以继续编辑
...
- **前置条件**：用户有未保存的更改
- **WHEN**: user clicks cancel button or closes editor with unsaved changes
...
The system SHALL display the editor in a side panel or inline within the card list, allowing users to see other cards while editing.
...
The system SHALL support keyboard shortcuts for common actions (Ctrl+S to save, Ctrl+B for bold, etc.).
...
The system SHALL provide a toolbar with icon buttons for formatting and actions.
...
The system SHALL allow editing multiple cards simultaneously in different panels or tabs.
...
**Test File**: `test/screens/card_editor_screen_desktop_test.dart`
