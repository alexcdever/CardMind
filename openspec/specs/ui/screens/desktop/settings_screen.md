...
**Related Tests**: `test/screens/settings_screen_desktop_test.dart`
...
This specification defines the desktop settings screen that provides comprehensive application configuration and management options organized into logical categories, optimized for desktop with side-panel or tabbed layout.

本规格定义了桌面端设置屏幕，提供按逻辑类别组织的全面应用程序配置和管理选项，针对桌面端优化，采用侧面板或标签页布局。
...
- **操作**：设置屏幕加载
- **THEN**: the system SHALL display settings in a two-panel layout with categories on the left and details on the right
- **预期结果**：系统应以两面板布局显示设置，左侧为类别，右侧为详情
- **AND**: show sections: Appearance, Devices, Synchronization, Data, About
- **并且**：显示部分：外观、设备、同步、数据、关于
...
The system SHALL provide inline device management interface.

系统应提供内联设备管理界面。

### Scenario: Show device manager inline
### 场景：内联显示设备管理器

- **GIVEN**: settings screen is displayed
- **前置条件**：设置屏幕已显示
- **WHEN**: user selects "Devices" category
- **操作**：用户选择"设备"类别
- **THEN**: the system SHALL show device management panel in the right pane
- **预期结果**：系统应在右侧面板显示设备管理面板
- **AND**: keep settings categories visible on the left
- **并且**：在左侧保持设置类别可见

### Scenario: Show current device info
### 场景：显示当前设备信息

- **GIVEN**: device settings are displayed
- **前置条件**：设备设置已显示
...
### Scenario: Show sync details inline
### 场景：内联显示同步详情

- **GIVEN**: settings screen is displayed
- **前置条件**：设置屏幕已显示
- **WHEN**: user selects "Synchronization" category
- **操作**：用户选择"同步"类别
- **THEN**: the system SHALL show sync details in the right pane
- **预期结果**：系统应在右侧面板显示同步详情
...
- **操作**：用户选择"开源许可证"
- **THEN**: the system SHALL display third-party licenses in a dialog or inline
- **预期结果**：系统应在对话框或内联显示第三方许可证
...
The system SHALL use a two-panel layout with categories on the left and settings details on the right.

系统应使用两面板布局，左侧为类别，右侧为设置详情。

### Inline Sub-Panels
### 内联子面板

The system SHALL display detailed settings (device manager, sync details) inline in the right pane instead of navigating to separate screens.

系统应在右侧面板内联显示详细设置（设备管理器、同步详情），而不是导航到单独的屏幕。

### Keyboard Navigation
### 键盘导航

The system SHALL support keyboard navigation between settings categories and options.

系统应支持在设置类别和选项之间进行键盘导航。
...
**Test File**: `test/screens/settings_screen_desktop_test.dart`
**测试文件**: `test/screens/settings_screen_desktop_test.dart`

**Screen Tests**:
**屏幕测试**:
- `it_should_show_two_panel_layout()` - Two-panel layout
...
- `it_should_show_device_manager_inline()` - Inline device manager
...
- `it_should_show_sync_details_inline()` - Inline sync details
- `it_should_show_sync_details_inline()` - 内联同步详情
...
- `it_should_support_keyboard_navigation()` - Keyboard navigation
...
- [ ] 所有配置选项正常工作
- [ ] Two-panel layout is intuitive
- [ ] 两面板布局直观
- [ ] Inline sub-panels work smoothly
- [ ] 内联子面板流畅工作
