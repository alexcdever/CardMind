...
**选项**:
- A: 不持久化，只保留当前状态（简单）
- B: 持久化最近 N 次状态变化（复杂，用于调试）

**建议**: 先实现 A，如果需要调试历史可以后续添加
...
**选项**:
- A: 仅支持 Stream（简单）
- B: 同时支持 Stream 和回调（灵活，但增加复杂度）

**建议**: 先实现 A，Stream 已足够灵活
...
**选项**:
- A: 仅在 Flutter 端 debounce（简单）
- B: 在 Rust 端也实现 debounce（减少跨语言调用）

**建议**: 先实现 A，如果性能不足再考虑 B
...
**选项**:
- A: 订阅所有状态变化（简单）
- B: 支持过滤特定状态（如只订阅 failed 状态）

**建议**: 先实现 A，Flutter 端可以用 `where()` 过滤
