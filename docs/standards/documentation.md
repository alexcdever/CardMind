...
目录规则：每个目录必须包含 `DIR.md`（根目录也需要）。正文 3 行以内说明定位，随后列出文件清单（文件名 + 地位 + 功能）。目录内的文件变更必须同步更新 `DIR.md`。

文件规则：仅对 `.dart` 与 `.rs` 文件适用。文件头三行注释说明 `input`、`output`、`pos`，分别用来介绍需要什么、输出什么和作用已经定位。修改文件需同步更新文件头与所属 `DIR.md`。
