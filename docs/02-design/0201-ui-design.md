...
```
首次启动应用 → 显示网络认证界面 → 用户输入网络ID → 点击加入按钮 → 验证网络ID → 验证成功 → 加入网络 → 进入主页
                    ↓                                        ↓
               点击跳过按钮                           验证失败 → 显示错误提示 → 用户重新输入
                    ↓
               进入离线模式 → 使用本地数据
                    ↓
      点击生成网络ID → 系统生成唯一网络ID → 自动加入新网络 → 进入主页
```
...
进入设置页面 → 进入网络设置区域
        ...
        ┌────────────┼────────────┐
        ↓            ↓            ↓
点击生成新网络ID  点击加入网络   点击编辑设备昵称
        ↓            ↓            ↓
生成新网络ID →     弹出输入框 →  弹出编辑框 →
自动加入新网络      输入网络ID    输入新昵称
        ↓            ↓            ↓
