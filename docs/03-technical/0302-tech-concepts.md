...
#### 1.1.3 Access Code机制
- **编码算法**: 使用URL安全的Base64编码，确保字符串可在URL中安全传输
- **数据结构**: Access Code包含网络ID、WebRTC连接信息、时间戳和设备标识
- **信息组成**:
  - 网络ID: 目标协作网络的唯一标识符
  - WebRTC连接信息: y-webrtc所需的信令服务器URL、房间配置等参数
  - 时间戳: 生成时间，用于连接有效性验证
  - 设备标识: 生成Access Code的设备ID，用于追踪来源
- **编码格式**: JSON对象序列化后使用Base64编码，长度通常为20-100字符
- **设计原则**: 不保存Access Code，每次需要时重新生成，确保连接信息最新。在家庭局域网场景中，支持同一Access Code被多个设备同时使用，无需防重放攻击机制
- **解析验证**: 解码后验证JSON结构完整性和字段有效性

#### 1.1.4 网络发现与加入
- **Access Code加入**: 通过解析Access Code获取网络ID和连接信息
...
- **Access Code输入**：提供Access Code输入界面，支持解析和验证
- **Access Code生成**：创建新网络时生成包含网络ID和连接信息的Access Code
- **网络加入**：通过Access Code解析出的网络ID和连接信息加入协作网络
...
### 2.3 Access Code生成与解析
- **生成流程**:
  - 获取当前网络ID和WebRTC配置信息
  - 收集当前设备标识和当前时间戳
  - 构建JSON对象包含所有必要信息
  - 使用Base64编码生成Access Code字符串
  - 返回Access Code并显示给用户（仅显示一次）
- **解析流程**:
  - 接收用户输入的Access Code字符串
  - 使用Base64解码获取JSON数据
  - 验证JSON结构完整性和字段有效性
  - 提取网络ID、WebRTC连接信息、时间戳和设备标识
  - 验证时间戳有效性（可选）
  - 返回解析结果用于网络加入
