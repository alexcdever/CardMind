...
1. 当前阶段 SHALL 采用全量自动挂接策略：创建或加入数据池时，系统自动将该用户所有已有笔记（含已软删除笔记）的 `noteId` 写入池元数据。
2. 用户创建新笔记后，系统自动将其挂接至当前所属数据池。
3. 全量自动挂接是当前阶段的简化设计，旨在避免用户在加入池后逐一选择笔记的额外操作负担。
4. 未来版本（V2）MAY 引入选择性挂接，允许用户在创建或加入池时选择哪些笔记进入池。
...
3. Pool 解散后，当前成员 MUST 仍可读取池内历史笔记，但 MUST NOT 编辑历史笔记或新增池内笔记。
