...
| 操作 | 移动端 | 桌面端 |
|---|---|---|
| 新建笔记 | FAB → EditorPage | 侧栏「新建笔记」→ 详情区编辑器 |
| 打开笔记 | 点击卡片 → EditorPage | 点击卡片 → 详情区编辑器 |
| 保存笔记 | EditorPage「完成」→ pop 并保存 | 详情区保存按钮 |
| 离开编辑 | PopScope 拦截 → 确认对话框 | 切换卡片时检测 dirty → 确认对话框 |
