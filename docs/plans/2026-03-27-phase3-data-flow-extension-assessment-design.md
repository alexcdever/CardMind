...
**保护范围**：
- 整个应用（每次启动都需要解锁）
- 基础笔记功能（创建/编辑/查看笔记）不需要解锁
- 数据池功能（创建/加入/管理池）必须解锁后才能使用

**解锁方式**：
- **首选**：生物识别（指纹/面容识别）
- **备选**：4-6位 PIN 码
- **策略**：用户自选，推荐生物识别但不强制
- **容错**：生物识别失败3次后，要求使用 PIN 码

**重置机制**：
- 用户忘记 PIN 码时，提供"重置应用锁"功能
- 重置需要用户确认（如输入账户密码或验证身份）
- 重置后保留所有数据，仅清除应用锁设置

**启用时机**：
- 首次启动时引导用户设置（可选，不强制）
- 首次尝试使用数据池功能时，如果未设置，强制引导设置
- 后续可在应用设置中修改或关闭

**架构设计**（前后端配合）：

**Flutter 端职责**：
- 调用 `local_auth` 进行生物识别
- 提供 PIN 码输入界面
- 调用 Rust API 进行验证和设置
- 管理应用锁UI状态（锁定/解锁界面）
- 在生物识别失败达到阈值（默认3次）时自动切换到 PIN 码
- 将解锁结果（成功/失败）反馈给 Rust，便于记录连续失败次数

**Rust 端职责**：
- 提供应用锁状态查询接口
- 提供验证接口（验证 PIN 码），并记录连续失败次数
- 提供设置接口（设置/修改 PIN 码和生物识别偏好）
- 提供重置接口（清除现有凭证，等待重新设置）
- **安全存储**：调用原生 API（Keychain/Keystore）安全存储凭证
- 管理应用锁业务逻辑，包括：
  - 记录当前会话是否已解锁
  - 记录连续失败次数（生物识别/ PIN）
  - 当失败次数达到阈值时锁定应用并要求等待一段时间或走重置流程

**与数据池功能的关系**：
- 应用锁是阶段3的**第一个功能**，作为后续数据池功能的**前置依赖**
...
- 管理员审批后变为"已通过"或"已拒绝"
- 申请人在审批前可以主动取消申请
- 超过72小时未处理的申请自动过期（72小时为建议值，可在实现阶段调整）
- 过期后申请人可立即重新提交申请
...
**解散后数据保留策略**：
- **数据保留**：池解散后，笔记数据永久保留在成员的本地设备上（受应用锁保护）
- **访问权限**：已解散池中的笔记，成员可永久读取，但不可编辑
- **恢复解散**：不提供"恢复解散"功能，简化设计
- **数据导出**：不强制要求解散前导出数据，但提供导出选项
...
**产品决策（已确定）**：
- **笔记归属**：用户退出后，其在池中创建的笔记**留在池中**，作为池的共享内容
- **退出后访问**：退出成员**不能再访问池中任何笔记**（包括自己创建的），受应用锁保护
- **重新加入**：重新加入后**恢复对所有池内笔记的访问**（包括退出期间其他成员创建的）
- **离线设备数据**：接受技术局限，离线设备可能保留本地数据副本，但应用锁提供基础防护
- **踢出操作（未来扩展）**：管理员踢出成员时，同样遵循上述规则
...
阶段3采用**顺序执行**策略，分为**4个功能依次实现**：

#### 功能1：应用锁功能（安全基线，前置依赖）

**目标**：实现应用级安全锁，作为后续数据池功能的前置依赖

**开发步骤**：
1. **Flutter 端**：
   - 设计应用锁的用户流程和界面
   - 调用 `local_auth` 实现生物识别解锁
   - 实现 PIN 码输入界面
   - 集成到应用启动流程
2. **Rust 端**：
   - 实现应用锁状态管理接口
   - 实现 PIN 码验证和设置接口
   - 调用原生 API 实现安全存储（Keychain/Keystore）
   - 实现重置接口
3. **联调测试**：
   - 验证前后端接口对接
   - 验证生物识别和 PIN 码流程
...
**目标**：实现"未解散池必须至少有1个管理员"的不变量校验

**开发步骤**：
1. 实现不变量校验逻辑（检查管理员数量）
2. 在 leave_pool 操作执行前增加校验
3. 在校验失败时返回明确的错误语义
4. 验证：规格要求满足

#### 功能3：池解散功能（生命周期闭环）

**目标**：实现数据池解散功能，完成生命周期闭环

**开发步骤**：
1. 实现池解散功能（更新 dissolved 状态）
2. 实现解散后操作限制（禁止编辑、新增等）
3. 实现解散信息记录（解散时间、解散人）
4. 验证：生命周期闭环完整

#### 功能4：加入申请审批流程（体验增强）

**目标**：完善数据池加入流程，实现完整的申请-审批机制

**开发步骤**：
1. 实现申请提交功能（创建 JoinRequest）
2. 实现申请审批功能（批准/拒绝）
3. 实现申请状态管理（待处理、已通过、已拒绝、已过期）
4. 集成到现有 P2P 网络层
5. 验证：池创建/加入流程完整可用
...
```
应用锁功能 → 管理员不变量校验 → 池解散功能 → 加入申请审批流程
    ↓
后续所有数据池功能依赖应用锁已启用
```

**启用检查点**：
- 功能1（应用锁）完成后，才能启用功能2/3/4的数据池功能
- 数据池相关界面和功能入口检查应用锁状态
- 应用锁未设置时，提示用户设置并跳转
- 应用锁已设置时，正常进入数据池功能
...
1. **审查并批准本设计文档**
2. **必要时更新规格文档**（特别是 pool.md，补充笔记归属、解散后数据保留等行为约束）
3. **按顺序启动开发**：
   - **功能1**：应用锁功能的设计与实现
   - **功能2**：管理员不变量校验的设计与实现
   - **功能3**：池解散功能的设计与实现
...
| 2026-03-27 | v1.3 | 移除"需要澄清的关键问题"章节；将讨论确定的产品决策（应用锁设计、笔记归属、解散后数据保留等）整合到对应设计章节；明确应用锁为前后端配合实现；将执行策略从"并行双轨"改为"4个功能顺序执行" | OpenCode |
