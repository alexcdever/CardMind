...
**技术实现**：
- Flutter `local_auth` 插件（生物识别）
- 自定义 PIN 码输入界面
- FlutterSecureStorage 或原生 Keychain/Keystore（安全存储凭证）
...
**周期1：管理员不变量校验（基础保障）**
1. 设计应用锁功能（Track A 并行开发）
2. 实现管理员不变量校验逻辑
3. 更新 leave_pool 操作，增加不变量校验
4. 验证：规格要求满足
...
| 2026-03-27 | v1.3 | 移除"需要澄清的关键问题"章节；将讨论确定的产品决策（应用锁设计、笔记归属、解散后数据保留等）整合到对应设计章节 | OpenCode |
