...
1. 文档体系结构性重构已完成一轮关键收口：`spec / plan / standard / agents` 边界已明确，spec 生命周期已切到“行为变更触发”，`docs/` 以外的 `DIR.md` 已全部删除；下一步如继续推进，只需按需清理少量高频历史误导文档。
...
1. ~~文档体系结构性重构第一轮收口~~ ✅ **已完成**（2026-04-09）
...
5. ~~Pool 前端一轮实质减负~~ ✅ **已完成**（2026-04-07）
...
6. ~~Pool 前端第一轮结构收口~~ ✅ **已完成**（2026-04-07）
   - `PoolPage` 主文件收成状态分发入口
...
9. ~~Phase 3 数据池治理与安全基线~~ ✅ **已完成**（2026-04-05）
...
10. ~~质量门禁基线清理~~ ✅ **已完成**（2026-04-05）
   - `flutter analyze` 通过
   - `cargo fmt --check` 通过
   - `cargo clippy` 通过
   - `dart run tool/quality.dart all` 通过

11. ~~应用锁前置能力~~ ✅ **已完成**（2026-04-05）
   - Rust 安全状态机与存储抽象
...
- 当前无工程阻塞；主要剩余问题是历史计划文档中的旧治理语义仍会在全仓搜索时形成噪音
