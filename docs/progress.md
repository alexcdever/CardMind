...
1. 产品定义收敛完成：`docs/product.md` 已重写为面向唯一用户的个人工具定义（痛点、交互原则、技术选型）。同步相关条款全部移除，当前阶段只做本地笔记体验。
2. v2/ 已实现：编辑器（appflowy_editor WYSIWYG Markdown）+ 笔记列表（时间倒序）+ 标签系统（编辑/筛选）+ 全文搜索（300ms debounce）+ 自动保存。
3. 待自用验证：在 Android 真机运行 v2，验证交互是否符合产品定义（打开即写、首行即标题、关闭即保存）。
