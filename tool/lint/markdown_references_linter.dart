...
void scanMarkdownFiles(Directory directory, Map<String, ReferenceInfo> references) {
...
void extractReferences(String content, Directory parentDir, Map<String, ReferenceInfo> references) {
...
List<String> verifyReferences(Directory projectRoot, Map<String, ReferenceInfo> references) {
  ...
  for (final entry in references.entries) {
    ...
    final absolutePath2 = path.normalize(path.join(referenceInfo.parentDir.path, originalPath));
    ...
    for (final file in files) {
      if (file is File && file.path.endsWith('.md') && path.basename(file.path) == path.basename(originalPath)) {
        found = true;
        break;
      }
    }
