...
YamlMap _workflow() {
  final source = File('.github/workflows/manual-build-artifacts.yml').readAsStringSync();
...
void main() {
  ...
  test('desktop jobs install current Rust runtime libraries', () {
    for (final platform in ['windows', 'linux']) {
      final steps = (_map(jobs[platform])['steps'] as YamlList)
          .map(_map)
          .toList();
      final rust = steps.firstWhere((step) => step['name'] == 'Build Rust library');
      expect(rust['working-directory'], 'rust-backend');
      expect(rust['run'], contains('cargo build --release'));
      final copy = steps.firstWhere((step) => step['name'] == 'Install Rust runtime library');
      expect(copy['run'], contains('cardmind_backend'));
      expect(copy['run'], contains(platform == 'windows' ? 'Test-Path' : 'test -s'));
    }
  });

  test('linux package preserves the complete bundle under the cardmind root', () {
    final steps = (_map(jobs['linux'])['steps'] as YamlList).map(_map).toList();
    final package = steps.firstWhere((step) => step['name'] == 'Package Linux artifact');
    expect(package['run'], contains('mkdir -p cardmind'));
    expect(package['run'], contains('-C . cardmind'));
    expect(package['run'], contains('tar -czf CardMind-Linux-x64.tar.gz -C . cardmind'));
    expect(package['run'], isNot(contains('-C build/linux/x64/release/bundle .')));
  });
  ...
  test('android build stages all supported JNI ABI directories', () {
    final steps = (_map(jobs['android'])['steps'] as YamlList).map(_map).toList();
    final build = steps.firstWhere((step) => step['name'] == 'Build Android Rust libraries');
  ...
  test('windows release is an Inno Setup exe rather than a zip', () {
    final steps = (_map(jobs['windows'])['steps'] as YamlList).map(_map).toList();
    final install = steps.firstWhere((step) => step['name'] == 'Compile Inno Setup installer');
    expect(install['run'], contains('ISCC.exe'));
    expect(install['run'], contains('/DSourceDir='));
    final artifact = steps.firstWhere((step) => step['name'] == 'Upload Windows artifact');
    expect(_map(artifact['with'])['path'], 'CardMind-Setup.exe');
    expect(steps.where((step) => '${step['run']}'.contains('Compress-Archive')), isEmpty);
  });

  test('release waits for all builds and uploads exact assets idempotently', () {
    final release = _map(jobs['release']);
    expect(release['if'], contains('success()'));
    final steps = (release['steps'] as YamlList).map(_map).toList();
    final download = steps.firstWhere((step) => step['uses'] == 'actions/download-artifact@v4');
    expect(_map(download['with'])['pattern'], 'release-*');
    final verify = steps.firstWhere((step) => step['name'] == 'Verify release assets');
    ...
    final publish = steps.firstWhere((step) => step['uses'] == 'softprops/action-gh-release@v2');
    expect(publish['uses'], 'softprops/action-gh-release@v2');
    expect(_map(publish['with'])['tag_name'], contains('steps.tag.outputs.tag'));
    ...
    expect(_map(publish['with'])['files'], contains('CardMind-Linux-x64.tar.gz'));
  });
}
