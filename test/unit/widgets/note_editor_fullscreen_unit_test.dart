...
void main() {
  group('EditorState Tests', () {
    test('it_should_UT-001: 测试初始状态创建（新建模式）', () {
      // 新建模式：card = null
    ...
    test('it_should_UT-002: 测试初始状态创建（编辑模式）', () {
    ...
    test('it_should_UT-004: 测试空内容检测（新建模式）', () {
    ...
    test('it_should_UT-005: 测试空内容检测（编辑模式）', () {
      // 输入：card ≠ null, content = "  "
      // 预期：不允许保存，显示错误提示
      const content = '  ';
      final isContentEmpty = content.trim().isEmpty;
    ...
    test('it_should_UT-006: 测试未保存更改检测', () {
    ...
    test('it_should_UT-007: 测试自动保存防抖', () {
      // 输入：连续输入 5 个字符
    ...
    test('it_should_UT-008: 测试回调类型定义', () {
  ...
  group('Content Validation Tests', () {
    test('it_should_测试完全空内容检测', () {
      const content = '';
      expect(content.trim().isEmpty, true);
    });

    test('it_should_测试空白字符内容检测', () {
      const content = '   ';
      expect(content.trim().isEmpty, true);
    });

    test('it_should_测试换行符内容检测', () {
      const content = '\n\n\n';
      expect(content.trim().isEmpty, true);
    });

    test('it_should_测试单字内容验证', () {
      const content = 'a';
      expect(content.trim().isEmpty, false);
    });

    test('it_should_测试文本内容验证', () {
      const content = '  hello world  ';
      expect(content.trim().isEmpty, false);
      expect(content.trim(), 'hello world');
    });

    test('it_should_测试特殊字符内容', () {
      const content = '😀🎉';
      expect(content.trim().isEmpty, false);
    });

    test('it_should_测试混合内容验证', () {
      const content = '  \n  hello  \n  ';
      expect(content.trim().isEmpty, false);
  ...
  group('Title Processing Tests', () {
    test('it_should_测试空标题自动填充', () {
      const title = '';
      final finalTitle = title.trim().isEmpty ? '无标题笔记' : title.trim();
      expect(finalTitle, '无标题笔记');
    });

    test('it_should_测试空白标题自动填充', () {
      const title = '   ';
      final finalTitle = title.trim().isEmpty ? '无标题笔记' : title.trim();
      expect(finalTitle, '无标题笔记');
    });

    test('it_should_测试有效标题保留', () {
      const title = '我的笔记';
      final finalTitle = title.trim().isEmpty ? '无标题笔记' : title.trim();
      expect(finalTitle, '我的笔记');
    });

    test('it_should_测试标题前后空格处理', () {
      const title = '  我的笔记  ';
      final finalTitle = title.trim().isEmpty ? '无标题笔记' : title.trim();
      expect(finalTitle, '我的笔记');
    });
